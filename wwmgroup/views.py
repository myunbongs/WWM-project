...
def groupcreate(request):
    ...
    if request.method == 'POST':
        ...
        group = WwmGroup.objects.create(user=user, leader_email=request.user.email,
                                        groupurl=WwmGroup.generate_random_slug_code)
        if form.is_valid():
            group = form.save()
            group.save()
...
def changeleader(request, group_url, nextleader):
    group = WwmGroup.objects.filter(groupurl=group_url)
    for g in group:
...
def banuser(request, group_url, ban_user):
    banuser = WwmGroup.objects.filter(groupurl=group_url, user=ban_user)
...
def grouplist(request, group_url):
    list = WwmGroup.objects.filter(groupurl=group_url)
...
def joingroup(request, group_url):
    group = get_object_or_404(WwmGroup, groupurl=group_url)
...
def showgroup(request, group_url):
    group = WwmGroup.objects.filter(groupurl=group_url, user=request.user)
